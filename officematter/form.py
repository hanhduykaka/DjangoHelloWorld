...
class UserForm(forms.ModelForm):
    ...
    confirm = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': "input--style-3", 'placeholder': "Confirm-password"}))
...
class UserProfileInfoForm(forms.ModelForm):
    bio = forms.CharField(label='your username', max_length=500, widget=forms.TextInput(
        attrs={'class': "input--style-3", 'placeholder': "Bio-information"}))
...
class TopicsForm(forms.ModelForm):
    top_name = forms.CharField(max_length=100, widget=forms.TextInput(
        attrs={'class': "input--style-3", 'placeholder': "Topic name"}))
...
class OrganizationForm(forms.ModelForm):
    Name = forms.CharField(max_length=200, label="Name", widget=forms.TextInput(
        attrs={'class': "form-control", 'placeholder': "Name"}))
    Type = forms.ModelChoiceField(queryset=Type.objects.all(
    ), widget=forms.Select(attrs={'class': "form-control"}))
    IsPublic = forms.BooleanField(widget=forms.CheckboxInput(
        attrs={'class': "form-check-input"}))
    Purpose = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'class': "form-control", 'placeholder': "Purpose"}))
